...
//Cette structure pour la recherche de K16
typedef struct message {
	long Hexacifer;
	int chiffrerBinaire[64];
	int BinairePermuterCifer[64];
	int leftcifer[32];
	int rightcifer[32];
	int rightciferE[48];
	int sbox6Bits[6];
...
//PC2 dans  key schedul 56 => 48 bits
...
static const long messageClaire = 0xE343B90758D3C2E3;

static const long chiffrerJuste = 0xCEA41165DB7632C4;
...
//transformation de  6 bit en 4 bit selon la S-box choisi
...
//retrouver le L16 et le L16 fauter
void obtenirR16L16(long hexa,Message *m){
	m->Hexacifer = hexa;
	hexatobinary(m->chiffrerBinaire,hexa,16);
	Permutation(m->BinairePermuterCifer, m->chiffrerBinaire, ip, 64);
	splitTab(m->BinairePermuterCifer,m->leftcifer,m->rightcifer, 32);
...
void extraire6Bits(Message *m, int position) {
	for(int i=0; i<6; i++) {
		m->sbox6Bits[i] = m->rightciferE[6*position+i];
	}
}
...
long rechercheExostive(const long LechiffrerJuste, const long *LeschiffrerFaux){
	...
	for(int w=0;w<32;w++){
		...
		xor(resultatxorLeft,juste.leftcifer,faux.leftcifer,32);
		Permutation(leftPmoin1, resultatxorLeft, pMoin1, 32);
		int bitfaux = bitFauter(juste.rightcifer,faux.rightcifer);
		printf("bit faux : %d\n", bitfaux);
		Permutation(juste.rightciferE,juste.rightcifer,e,48);
		Permutation(faux.rightciferE,faux.rightcifer,e,48);
		int resSbox[4] = {0};
...
//obtenir les 56 bit juste de la clef de chiffrement
...
int main(){
	...
	long K_64 = getK64bitParite(messageClaire, chiffrerJuste, K_16);
	printf("k16: %lx\n", K_16); 
	printf("K : %lx\n", K_64);
	return 0;
}
